...
- (void)subscribeDisconnectedListener:(CDVInvokedUrlCommand *)command{
    self.disconnectCallbackID = command.callbackId;
}
- (void)unsubscribeDisconnectedListener:(CDVInvokedUrlCommand *)command{
    self.disconnectCallbackID = nil;
}
        ...
        if accessory != nil{
            if accessory.connectionID == self.accessory.connectionID{
                ...
                if self.disconnectCallbackID != nil{
                    ...
                    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:accessoryDetailsArray];
                    [pluginResult setKeepCallbackAsBool:true];
                    [self.commandDelegate sendPluginResult:pluginResult callbackId:self.disconnectCallbackID];
                }
            }
        }
    }
